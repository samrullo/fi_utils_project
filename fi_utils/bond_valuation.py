...
def set_the_day_of_the_date(adate:datetime.date,day:int):
    try:
        ...
        logging.info(f"breached month days range for the month {adate} with days {day}: {e}")
...
def find_next_coupon_date(
    ...
    while possible_coupon_date.year==adate.year:
        possible_coupon_date=set_the_day_of_the_date(possible_coupon_date-datetime.timedelta(days=days_per_year/freq),maturity.day)
    ...
    possible_coupon_dates_adate_diff_list=[(thedate-adate).days for thedate in possible_coupon_dates_in_the_year]

    # next coupon dates from as of date are those where possible coupon date minus adate is non-negative
    next_coupon_dates_in_the_year=[thedate for idx,thedate in enumerate(possible_coupon_dates_in_the_year) if possible_coupon_dates_adate_diff_list[idx]>=0 ]

    # next coupon date is the first item in the next coupon dates list
    return next_coupon_dates_in_the_year[0]

def calculate_pv_from_ytm(ytm:float, coupon_rate:float,adate:datetime.date,maturity:datetime.date, days_per_year:int=365, freq:int=2, principal_amount:float=100.0)->float:
    """
    ...
    for period in range(1,no_of_periods+1):
        pv+=(coupon_rate/2)/(1+ytm/100)**(time_to_next_cpn_date+period/freq)
    pv += principal_amount/(1+ytm/100)**(time_to_next_cpn_date+no_of_periods/freq)
    return pv

from scipy.optimize import fsolve
import numpy as np
def calc_ytm_of_bond(price:float, coupon_rate:float, adate:datetime.date, maturity:datetime.date, days_per_year:int=365, freq:int=2, principal_amount:float=100):
    def objective_func(ytm:np.ndarray):
        ytm=float(ytm)
        return (price - calculate_pv_from_ytm(ytm, coupon_rate,adate,maturity,days_per_year,freq,principal_amount))**2
