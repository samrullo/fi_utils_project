...
class MyTestCase(unittest.TestCase):
    def test_find_next_coupon_date(self):
        ...
        freq=2
        next_cpn_date=find_next_coupon_date(adate,maturity,freq=freq, days_per_year=365)
        print(f"next coupon date when adate is {adate}, maturity is {maturity}, freq is {freq} : {next_cpn_date}")
    ...
    def test_calculate_pv_from_ytm(self):
        ...
        ytm=4.0
        coupon_rate=5.0
        pv=calculate_pv_from_ytm(ytm, coupon_rate, datetime.date(2025,4,17), datetime.date(2048,9,25))
